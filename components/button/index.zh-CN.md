...
在 Ayla Design Vue 中我们提供了五种按钮。

- 主要按钮：用于主行动点，一个操作区域只能有一个主要按钮。
- 次要按钮（默认）：用于没有主次之分的一组行动点。
- 文字按钮：用于最次级的行动点。
- 危险按钮：用于危险操作，一般需要二次确认。
- 文字危险按钮：用于危险操作，一般需要二次确认。
...
| type | 设置按钮类型 | `primary` \| `secondary` \| `text` \| `danger` \| `text-danger` | `secondary` |  |
